...
ask_hash_target_default_variante() {
  local answer
  while true; do
    read -r -p "Hash target (master/variante) [variante]: " answer
    answer="${answer,,}"
    answer="${answer:-variante}"
    if [[ "$answer" == "master" || "$answer" == "variante" ]]; then
      echo "$answer"
      return 0
    fi
    echo "Errore: inserisci master o variante." >&2
  done
}
...
VARIANTE_FILE="$(ls -1t generazione_reperti/immagini/variante_*.png | head -n 1)"
