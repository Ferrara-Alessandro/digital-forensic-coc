...
read_nonempty ID_LAB "Inserisci l'ID del laboratorio (come sul decreto): "

echo "Operazione avviata per il reperto: $ID_REPERTO (accettazione lab $ID_LAB)..."
...
docker exec -e CORE_PEER_MSPCONFIGPATH="$LAB_ADMIN_MSP" peer0.lab.it \
  ...
  -c "{\"function\":\"RiceviInLaboratorio\",\"Args\":[\"$ID_REPERTO\",\"$ID_LAB\"]}"
