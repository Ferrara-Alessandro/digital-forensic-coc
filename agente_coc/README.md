...
server.py  (FastMCP)
    ↓  subprocess / generazione_reperti
generazione_reperti  →  testi (Ollama) e immagini (Stable Diffusion)
...
### Generazione reperti (test)

| Tool | Descrizione |
|------|-------------|
| `genera_evidenza_testo` | File in `generazione_reperti/testi/` (Ollama o template) |
| `genera_evidenza_immagine` | PNG in `generazione_reperti/immagini/` (Stable Diffusion) |
